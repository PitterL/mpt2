...
/**
 * \brief Set led_position_mode pull mode
 *
 * Configure pin to pull up, down or disable pull mode, supported pull
 * modes are defined by device used
 *
 * \param[in] pull_mode Pin pull mode
 */
static inline void led_position_mode_set_pull_mode(const enum port_pull_mode pull_mode)
{
	PORTB_set_pin_pull_mode(7, pull_mode);
}

/**
 * \brief Set led_position_mode data direction
 *
 * Select if the pin data direction is input, output or disabled.
 * If disabled state is not possible, this function throws an assert.
 *
 ...
 *                      (low power state)
 */
static inline void led_position_mode_set_dir(const enum port_dir dir)
{
	PORTB_set_pin_dir(7, dir);
}

/**
 * \brief Set led_position_mode input/sense configuration
 *
 * Enable/disable led_position_mode digital input buffer and pin change interrupt,
 * select pin interrupt edge/level sensing mode
 *
 * \param[in] isc PORT_ISC_INTDISABLE_gc    = Iterrupt disabled but input buffer enabled
 *                PORT_ISC_BOTHEDGES_gc     = Sense Both Edges
 ...
 *                PORT_ISC_LEVEL_gc         = Sense low Level
 */
static inline void led_position_mode_set_isc(const PORT_ISC_t isc)
{
	PORTB_pin_set_isc(7, isc);
}

/**
 * \brief Set led_position_mode inverted mode
 *
 * Enable or disable inverted I/O on a pin
 *
 * \param[in] inverted true  = I/O on led_position_mode is inverted
 *                     false = I/O on led_position_mode is not inverted
 */
static inline void led_position_mode_set_inverted(const bool inverted)
{
	PORTB_pin_set_inverted(7, inverted);
}

/**
 * \brief Set led_position_mode level
 *
 * Sets output level on a pin
 *
 * \param[in] level true  = Pin level set to "high" state
 *                  false = Pin level set to "low" state
 */
static inline void led_position_mode_set_level(const bool level)
{
	PORTB_set_pin_level(7, level);
}

/**
 * \brief Toggle output level on led_position_mode
 *
 * Toggle the pin level
 */
static inline void led_position_mode_toggle_level()
{
	PORTB_toggle_pin_level(7);
}

/**
 * \brief Get level on led_position_mode
 *
 * Reads the level on a pin
 */
static inline bool led_position_mode_get_level()
{
