...
qtouch_config_t tsl_qtouch_def = {
	...
	.chip_cb = {
		.get_sigrow = tslink_get_signature_row_data,
	#ifdef MPTT_FUSE_CHECK
		.get_fuse = tslink_get_fuse_data,
	#endif
	},
#endif
	...
	.map_cb = {
		...
		.to_channel = tslink_get_sensor_key_mapping,
		/* channel node to sensor key index mapping */
		.to_sensor = tslink_get_channel_node_mapping,
	}
};