...
void tslib_key_sensors_update_timer(uint16_t interval)
{
    qtm_update_qtlib_timer(interval);

#ifdef TOUCH_API_GESTURE_2D_H
    qtm_update_gesture_2d_timer(interval);
#endif
}
...
uint8_t tslib_num_channel_nodes(void)
{
    return GET_ACQ_SET_i(qtm_acq_node_group_config)->num_channel_nodes/* errata, here some touch lib will named`num_sensor_nodes */;
}
