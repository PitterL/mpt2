...
#endif
#ifdef OBJECT_T126
	{API_DEF_QTM_AUTOSCAN_DRIFT_PERIOD_MS, &measurement_period_idle_drift, sizeof(measurement_period_idle_drift), 0 },
...
static inline ssint tch_config_rw(const tch_config_callback_t *param, void *buf, u8 size, u8 index, u8 rw)
{
	void *dst, *src;
	size_t trunk_size = 0;
	ssint result;
...
ssint tslink_config_op(u8 type, void *buf, size_t size, u8 index, u8 rw)
{
...
ssint tslink_read_ref_signal_cap(u8 index, cap_sample_value_t *cval)
{
	const qtm_touch_key_group_config_t * const qttkg = &qtlib_key_grp_config_set1;
	const qtm_touch_key_data_t * const qtkds = &qtlib_key_data_set1[0];
...
u16 tslink_get_sensor_base_ref(void)
{
...
uint8_t tslink_fuse_check(void)
{
	return fuse_check();
}
#endif
