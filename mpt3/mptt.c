...
/*
	...
	v23(1.1): 
		 ...
		 <8> Change mpt_api_request_irq() ENTER_CRITICAL zone and placed into each call before sleep
	v23(2.0):
	...
	v24(2.1):
		...
		<14> change sleep logic: mptt_sleep()->tsl_sleep()->touch_sleep()->touch_process_lowpower()=>sleep_cpu(), it will check tslink_get_chip_state() and mptt_get_bus_state()
	...
	v25(1.0):
		...
		<4> tslink_get_number_channel_nodes() and tslink_get_number_key_sensors() for fast access sensor count at main loop to save power comsuption
...
typedef struct objects_config {
	...
	object_t8_t t8;
#endif
#if (defined(OBJECT_T9) || defined(OBJECT_T9_DUMMY))
...
#ifdef OBJECT_T37
	{	MXT_DEBUG_DIAGNOSTIC_T37, /*start_address*/(u16)-1, sizeof(struct object_t37) - 1, /*instances_minus_one*/0, /*num_report_ids*/0	},
#endif
#ifdef OBJECT_T44	
	{	MXT_SPT_MESSAGECOUNT_T44, /*start_address*/(u16)-1, sizeof(struct object_t44) - 1, /*instances_minus_one*/0, /*num_report_ids*/0	},
#endif
#ifdef OBJECT_T5
	{	MXT_GEN_MESSAGE_T5, /*start_address*/(u16)-1, sizeof(struct object_t5) - 1, /*instances_minus_one*/0, /*num_report_ids*/0	},
#endif
#ifdef OBJECT_T6	
	{	MXT_GEN_COMMAND_T6, /*start_address*/(u16)-1, sizeof(struct object_t6) - 1, /*instances_minus_one*/0, /*num_report_ids*/MXT_GEN_COMMAND_T6_RIDS	},
#endif
#ifdef OBJECT_T38
	{	MXT_SPT_USERDATA_T38, /*start_address*/(u16)-1, sizeof(struct object_t38) - 1, /*instances_minus_one*/0, /*num_report_ids*/0	},
#endif
#ifdef OBJECT_T7	
	{	MXT_GEN_POWER_T7, /*start_address*/(u16)-1, sizeof(struct object_t7) - 1, /*instances_minus_one*/0, /*num_report_ids*/0	},
#endif	
#ifdef OBJECT_T8	
	{	MXT_GEN_ACQUIRE_T8, /*start_address*/(u16)-1, sizeof(struct object_t8) - 1, /*instances_minus_one*/0, /*num_report_ids*/0	},
#endif	
#if (defined(OBJECT_T9) || defined(OBJECT_T9_DUMMY))
	{	MXT_TOUCH_MULTI_T9, /*start_address*/(u16)-1, sizeof(struct object_t9) - 1, /*instances_minus_one*/MXT_TOUCH_MULTI_T9_INST - 1, /*num_report_ids*/MXT_TOUCH_MULTI_T9_RIDS	},
#endif
#ifdef OBJECT_T15	
	{	MXT_TOUCH_KEYARRAY_T15, /*start_address*/(u16)-1, sizeof(struct object_t15) - 1, /*instances_minus_one*/MXT_TOUCH_KEYARRAY_T15_INST - 1, /*num_report_ids*/MXT_TOUCH_KEYARRAY_T15_RIDS	},
#endif	
#ifdef OBJECT_T18
	{	MXT_SPT_COMMSCONFIG_T18, /*start_address*/(u16)-1, sizeof(struct object_t18) - 1, /*instances_minus_one*/0, /*num_report_ids*/0	},
#endif
#ifdef OBJECT_T25
	{	MXT_SPT_SELFTEST_T25, /*start_address*/(u16)-1, sizeof(struct object_t25) - 1, /*instances_minus_one*/0, /*num_report_ids*/MXT_SPT_SELFTEST_T25_RIDS	},
#endif
#ifdef OBJECT_T104
	{	MXT_SPT_AUXTOUCHCONFIG_T104, /*start_address*/(u16)-1, sizeof(struct object_t104) - 1, /*instances_minus_one*/MXT_SPT_AUXTOUCHCONFIG_T104_INST - 1, /*num_report_ids*/0	},
#endif
#ifdef OBJECT_T109
	{	MXT_SPT_SELFCAPGLOBALCONFIG_T109, /*start_address*/(u16)-1, sizeof(struct object_t109) - 1, /*instances_minus_one*/0, /*num_report_ids*/MXT_SPT_SELFCAPGLOBALCONFIG_T109_RIDS	},
#endif
#ifdef OBJECT_T111	
	{	MXT_SPT_SELFCAPCONFIG_T111, /*start_address*/(u16)-1, sizeof(struct object_t111) - 1, /*instances_minus_one*/MXT_SPT_SELFCAPCONFIG_T111_INST - 1, /*num_report_ids*/0	},
#endif
#ifdef OBJECT_T126
	{	MXT_SPT_LOWPOWERIDLECONFIG_T126, /*start_address*/(u16)-1, sizeof(struct object_t126) - 1, /*instances_minus_one*/0, /*num_report_ids*/MXT_SPT_LOWPOWERIDLECONFIG_T126_RIDS	},
#endif
};
...
static ssint mpt_chip_backup(/*data_crc24_t*/ void *crc_ptr, u8 ccp);
...
ssint mpt_api_chip_init(const void *tsl_ptr)
{
	...
	ibinf->matrix_xsize = QT_MATRIX_X_SIZE(cfm->tsl->qtdef);
	ibinf->matrix_ysize = QT_MATRIX_Y_SIZE(cfm->tsl->qtdef);
...
#ifdef OBJECT_T25
	result = object_api_t25_selftest_porstage();
#endif
...
static void mpt_chip_reset(void)
{
	...
	if (HAL_INTERFACE_VAL(cfm->tsl->hal,reset)) {
		HAL_INTERFACE_VAL(cfm->tsl->hal,reset)();
	}
}
...
static void mpt_chip_calibrate(void)
{
	...
	if (QTOUCH_API_CALLBACK(cfm->tsl->api, calibrate)) {
		QTOUCH_API_CALLBACK(cfm->tsl->api, calibrate)();
	}
}
...
static ssint mpt_chip_backup(/*data_crc24_t*/ void *crc_ptr, u8 ccp)
{
	...
	if (!HAL_INTERFACE_VAL(cfm->tsl->hal, save_cfg))
		return -2;
	
	result = HAL_INTERFACE_VAL(cfm->tsl->hal, save_cfg)((u8 *)&ibreg->noncfg,  sizeof(ibreg->noncfg) + sizeof(ibreg->cfg), ccp);
	if (result) {
		/* something error */
		result = -3;
	} else {
...
static ssint mpt_chip_load_config(void)
{
	...
	if (!HAL_INTERFACE_VAL(cfm->tsl->hal, load_cfg))
		return -2;
		
	result = HAL_INTERFACE_VAL(cfm->tsl->hal, load_cfg)((u8 *)&ibreg->noncfg,  sizeof(ibreg->noncfg) + sizeof(ibreg->cfg));
	if (result) {
		/* something error */
		result = -3;
...
static void mpt_chip_assert_irq(u8 assert, bool retrigger)
{
	...
	if (HAL_INTERFACE_VAL(cfm->tsl->hal, assert_irq))
		HAL_INTERFACE_VAL(cfm->tsl->hal, assert_irq)(assert, retrigger);	
}
...
{
#ifdef OBJECT_T25
	if (mpt_api_touch_state_idle()) {
		object_api_t25_selftest_prestage();
	}
#endif
...
/*
    ...
    @sensor: key sensor number
    @cv: the `rsd` data, if import Null, that means to test whether `rsd` data is required
		@busy: the ref/signal is 16bits, data may crashed when bus is reading and update.
...
#ifdef OBJECT_T25
	result = object_api_t25_selftest_poststage(sensor, cv);
	if (result == 0) {
		checked = 0;
	}
#endif
...
/*
 * \brief whether the test is in progress
 * @Return: the test op in T25, zero means idle
 */
u8 mpt_api_get_selftest_op(void)
{
#ifdef OBJECT_T25
	return object_api_t25_test_inprogress();
#else
	return 0;
#endif
}
...
/*
 * \brief read value from link layer
 * @Return: ZERO means success, other value is error code
 */
ssint mpt_api_config_op(u8 type, void *buf, u8 size, u8 index, u8 rw)
{
	return tslink_config_op(type, buf, size, index, rw);
}

u8 mpt_api_read_config_byte(u8 type)
{
	return tslink_read_config_byte(type);
}
