...
u16 t37_get_data(u8 cmd, u8 channel, ref_t reference, sig_t signal, cap_t cap)
{
...
void t37_put_data(t37_data_t *ptr, u8 cmd, u8 page, u8 channel, u16 value)
{
	...
	switch(cmd) {
		...
		case MXT_DIAGNOSTIC_MC_SIGNAL:
			pos = channel;
			copy_data_to_target_page(cmd, page, (u8)(pos << 1), &value, sizeof(value));
		break;
#ifdef OBJECT_T111
		case MXT_DIAGNOSTIC_SC_REF:
		case MXT_DIAGNOSTIC_SC_SIGNAL:
...
void t37_set_rsf_data(u8 cmd, u8 page, u8 channel, ref_t reference, sig_t signal, cap_t cap)
{	
	t37_data_t *ptr = &t37_data_status;
	u16 data;
...
#ifdef OBJECT_T126
void t37_set_low_power_data(u8 cmd, u8 page, u8 channel, ref_t reference, sig_t signal, cap_t cap)
{
	...
	data.delta = (dlt_t)((s16)signal- (s16)reference);
	data.ref = reference;
	data.signal = signal;
