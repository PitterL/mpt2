...
void t9_data_sync(t9_data_t *ptr, u8 rw)
{
	...
	txx_cb_param_t params_touch[] = {
		...
		{ API_SURFACE_CS_FILT_CFG, &mem->movfilter, sizeof(mem->movfilter) },
#endif
#ifdef API_SURFACE_CS_MIN_CONTACT
		{ API_SURFACE_CS_MIN_CONTACT, &mem->amphyst, sizeof(mem->amphyst) },
#endif
	};
...
void t9_report_status(u8 rid, const t9_point_status_t *pt, u8 res_bit, const mpt_api_callback_t *cb)
{
	...
	if (res_bit <= 10) {
		message.data[1] = (u8)(pt->pos.x >> 2);
		message.data[2] = (u8)(pt->pos.y >> 2);
		message.data[3] = (u8)(((pt->pos.x & 0x3F) << 2) | (pt->pos.y & 0x3F));
	} else {
		message.data[1] = (u8)(pt->pos.x >> 4);
		message.data[2] = (u8)(pt->pos.y >> 4);
		message.data[3] = (u8)(((pt->pos.x & 0xF) << 4) | (pt->pos.y & 0xF));
	}
...
bool object_t9_is_enabled(u8 inst_id)
{
	t9_data_t *ptr = &t9s_data_status[0];
	object_t9_t *mem;

	if (inst_id < MXT_TOUCH_MULTI_T9_INST) {
		mem = (object_t9_t *)ptr[inst_id].common.mem;
		return !!(mem->ctrl & MXT_T9_CTRL_ENABLE);
	}
