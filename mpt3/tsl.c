...
{
	.sync = mpt_api_config_op,
#ifdef OBJECT_T6
	.calibrate = mpt_api_calibrate,
#endif
	// .idle = mpt_api_touch_state_idle,
	// .suspend = mpt_api_touch_suspend
};
...
{
	u8 sensor_count = mpt_api_get_number_key_sensors();
	...
	for (i = 0; i < qdef->num_button; i++) {
		if (!btn[i].node.size) {
			if (i == 0) {	//All nodes assigned to button 0
				btn[i].node.origin = 0;
				btn[i].node.size = mpt_api_get_number_key_sensors();
			} else {
				break;
			}
		}
...
void init_slider_nodes(qtouch_config_t *qdef)
{
	...
	u8 num_scrollers = mpt_api_read_config_byte(API_NUM_SLIDERS);
	u8 val, i;
	...
	for ( i = 0; i < num_scrollers && i < qdef->num_surfaces_slider; i++) {
		if (!(sursld[i].nodes[NODE_X].size || sursld[i].nodes[NODE_Y].size)) {
			...
			sursld[i].nodes[NODE_Y].origin = tslink_read_group_config_byte(API_SLIDER_START_KEY, i);
			sursld[i].nodes[NODE_Y].size = tslink_read_group_config_byte(API_SLIDER_NUM_KEYS, i);

            val = tslink_read_group_config_byte(API_SLIDER_RESOL_DB, i);
...
void init_surface_node(qtouch_config_t *qdef)
{
	...
	if (!(sursld->nodes[NODE_X].size && sursld->nodes[NODE_Y].size)) {
		...
		sursld->nodes[NODE_X].origin = mpt_api_read_config_byte(API_SURFACE_CS_START_KEY_V);	//should start first
		sursld->nodes[NODE_X].size = mpt_api_read_config_byte(API_SURFACE_CS_NUM_KEYS_V);
		sursld->nodes[NODE_Y].origin = mpt_api_read_config_byte(API_SURFACE_CS_START_KEY_H);
		sursld->nodes[NODE_Y].size = mpt_api_read_config_byte(API_SURFACE_CS_NUM_KEYS_H);
	}
	
	// Resolution
	val = mpt_api_read_config_byte(API_SURFACE_CS_RESOL_DB);
...
/**
 * \brief touch software layer touch process, 
 */
void tsl_touch_process(void)
{
	mpt_api_touch_process();
}
 ...
 */
	//
void tch_ref_signal_update(bool busy)
{
	...
	for (i = 0; i < sensor_count; i++) {
		// Fetch `rsd` data
		result = mpt_api_read_ref_signal_cap(i, &cvalue);
		if (result == 0) {
			// Dispatch `rsd` data
			mpt_api_set_sensor_data(i, &cvalue, busy);
		}
	}
#endif
}
...
{
#ifdef OBJECT_T6
	u8 state = mpt_api_get_chip_state();
	if (state) {
...
{
	u8 button_count = mpt_api_get_number_key_sensors();
	u8 i;
	ssint result;
	
	for (i = 0; i < button_count; i++) {
		result = mpt_api_read_button_state(i);
		if (result >= 0) {
...
{
	u8 slider_count = mpt_api_get_number_slider_sensors();
...
#define SLIDER_FINGER_ID 0
		result = mpt_api_read_slider_state(i, &point);
		if (result == 0) {
...
#define SURFACE_FINGERS 1
	...
	for (i = 0; i < SURFACE_FINGERS; i++) {
		result = mpt_api_read_surface_state(i, &point);
		if (result == 0) {
...
{	
	...
	tch_ref_signal_update(mptt_get_bus_state() == BUS_READ);
